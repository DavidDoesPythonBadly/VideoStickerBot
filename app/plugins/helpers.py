...
class Helpers(BotAPI):
    ...
    async def user_settings(self) -> Union[tuple[bool, bool], tuple[str, InlineKeyboardMarkup]]:
    ...
    @staticmethod
    async def extract_emojis(text: Union[str, Message]) -> str:
    ...
    async def send_webm(message: Message):
        ...
        file = f"downloads/{message.from_user.id}_{message.id}.webm"
